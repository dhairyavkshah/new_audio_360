...
- **Music Folder Selection**: On first launch, users select their music folder using Android's Storage Access Framework (SAF). Songs load exclusively from the selected folder. Settings allows changing the folder at any time. Web/iOS users can use demo songs.
